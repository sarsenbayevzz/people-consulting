...
def grouped_bar(df, x_col, y_cols, colors, title="", height=360, names=None):
    fig = go.Figure()
...
# =========================================================
    ...
    with tab_company:
        ...
        if co_row.empty:
            ...
            # 2025 data for company
            co25_row = filtered_c25[filtered_c25["company_name"] == company] if not filtered_c25.empty and "company_name" in filtered_c25.columns else pd.DataFrame()
            co25 = co25_row.iloc[0] if not co25_row.empty else None

            # --- KPI ROW 1: perception ---
            k1, k2, k3, k4, k5 = st.columns(5)
            ...
            # ── ROW 1: Структура восприятия  +  Год к году ──────────────
            col1, col2 = st.columns(2)
            ...
            with col3:
                ...
                st.caption("Где компания стоит среди всех участников рейтинга")
                ...
                if not selected.empty:
                    fig.add_trace(go.Scatter(
                        ...
                        mode="markers+text",
                        marker=dict(color=COLOR_ACCENT, size=18, line=dict(color=COLOR_TEXT, width=2)),
                        text=[company],
                        textposition="top center",
                ...
                # Квадрантные линии
                fig.add_vline(x=want_med,  line_dash="dot", line_color=COLOR_BORDER, line_width=1)
                fig.add_hline(y=nwant_med, line_dash="dot", line_color=COLOR_BORDER, line_width=1)
                # Подписи квадрантов
                ...
                for qtext, qx, qy, qcolor in [
                    ...
                    fig.add_annotation(
                        ...
                        showarrow=False,
                        font=dict(color=qcolor, size=13, family="Syne"),
                        opacity=0.7,
                    )
                update_layout_with_defaults(
            ...
            with col4:
                ...
                st.caption("Место компании в трёх независимых рейтингах")

                total_cos = len(filtered_c26)
                rank_items = [
                    ("Ранг по Want",        "rank_want",         "want_pct_change",         COLOR_WANT,      "Чем ниже номер — тем привлекательнее"),
                    ("Ранг по Not Want",    "rank_not_want",     "not_want_pct_change",     COLOR_NOT_WANT,  "Чем ниже номер — тем хуже репутация"),
                    ("Ранг по Unknown",     "rank_unknown_brand","unknown_brand_pct_change", COLOR_UNKNOWN,   "Чем ниже номер — тем менее известна"),
                ]

                for rank_label, rank_col, change_col, rank_color, hint in rank_items:
                        ...
                        st.markdown(
                            ...
                            ">
                                ...
                                <div style="display:flex;align-items:baseline;gap:12px;margin-top:4px;">
                                    <span style="font-size:2.4rem;font-weight:800;font-family:Syne,sans-serif;color:{bar_color};">
                                        #{rank_int}
                                    </span>
                                ...
                                <div style="font-size:0.72rem;color:#6B7280;margin-top:4px;">{hint}</div>
                            </div>
                            """,
                            unsafe_allow_html=True
                        )
            ...
            # ── ROW 3: Конверсия аудитории (воронка) ────────────────────
            st.markdown("### Конверсия аудитории")
            st.caption("Из тех, кто знает компанию — сколько хотят, сомневаются, или против")

            total_resp = co.get("total_responses")
            if total_resp is not None and pd.notna(total_resp) and total_resp > 0:
                total_resp = int(total_resp)
                unknown_n  = int(round(co.get("unknown_brand_pct", 0) * total_resp))
                known_n    = total_resp - unknown_n
                want_n     = int(round(co.get("want_pct", 0) * total_resp))
                unsure_n   = int(round(co.get("unsure_pct", 0) * total_resp))
                notwant_n  = int(round(co.get("not_want_pct", 0) * total_resp))

                # Conversion rate: из знающих — сколько хотят
                conv_rate = want_n / known_n * 100 if known_n > 0 else 0

                # KPI mini-row
                fc1, fc2, fc3, fc4 = st.columns(4)
                fc1.metric("Всего ответили",    f"{total_resp:,}")
                fc2.metric("Знают компанию",    f"{known_n:,}", delta=f"{known_n/total_resp*100:.0f}% от всех")
                fc3.metric("Хотят работать",    f"{want_n:,}",  delta=f"{conv_rate:.0f}% из знающих")
                fc4.metric("Против",            f"{notwant_n:,}", delta=f"{notwant_n/known_n*100:.0f}% из знающих" if known_n > 0 else None, delta_color="inverse")

                # Funnel chart
                funnel_df = pd.DataFrame({
                    "Этап": [
                        f"Ответили на опрос ({total_resp:,})",
                        f"Знают компанию ({known_n:,})",
                        f"Хотят работать ({want_n:,})",
                    ],
                    "Значение": [total_resp, known_n, want_n],
                    "Цвет": [COLOR_MUTED, COLOR_ACCENT, COLOR_WANT],
                })
                fig = go.Figure(go.Funnel(
                    y=funnel_df["Etap"] if "Etap" in funnel_df else funnel_df["Этап"],
                    x=funnel_df["Значение"],
                    textinfo="value+percent previous",
                    marker=dict(color=funnel_df["Цвет"].tolist()),
                ...
                update_layout_with_defaults(
                    ...
                    yaxis=dict(tickfont=dict(color=COLOR_TEXT, size=PLOT_TICK_SIZE)),
                )
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.info("Нет данных о количестве ответивших")

            st.markdown("---")

            # ── ROW 4: 2025 vs 2026 (полный профиль) ────────────────────
            if co25 is not None:
                st.markdown("### Профиль 2025 vs 2026")
                st.caption("Как изменился полный профиль восприятия компании за год")

                metrics_26 = [co.get("want_pct",0), co.get("not_want_pct",0), co.get("unsure_pct",0), co.get("unknown_brand_pct",0)]
                metrics_25 = [co25.get("want_pct",0), co25.get("not_want_pct",0), co25.get("unsure_pct",0), co25.get("unknown_brand_pct",0)]
                metric_names = ["Want %", "Not Want %", "Unsure %", "Unknown %"]
                bar_colors   = [COLOR_WANT, COLOR_NOT_WANT, COLOR_UNSURE, COLOR_UNKNOWN]
                ...
                fig.add_trace(go.Bar(
                    ...
                    x=metric_names,
                    y=metrics_25,
                    marker_color=[hex_to_rgba(c, 0.33) for c in bar_colors],  # приглушённые цвета для 2025
                    marker_line=dict(color=bar_colors, width=2),
                    text=[f"{v:.1%}" for v in metrics_25],
                    textposition="outside",
                    textfont=dict(color=COLOR_MUTED, size=PLOT_TEXT_SIZE - 2),
                    hovertemplate="<b>%{x}</b><br>2025: %{y:.1%}<extra></extra>",
                ))
                fig.add_trace(go.Bar(
                    name="2026",
                    x=metric_names,
                    ...
                    text=[f"{v:.1%}" for v in metrics_26],
                    textposition="outside",
                    textfont=dict(color=COLOR_TEXT, size=PLOT_TEXT_SIZE - 2),
                    hovertemplate="<b>%{x}</b><br>2026: %{y:.1%}<extra></extra>",
                ))
                update_layout_with_defaults(
